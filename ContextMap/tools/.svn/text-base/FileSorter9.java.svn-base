...
public class FileSorter9 extends Thread {
	...
	public static void main(String args[]) {
		...
		FileSorter9 fileSorter = new FileSorter9(inputPath,outputPath,tmpFolderPath,columnsToSortFor,chunkSize,delimiter,numericalSort);
		fileSorter.sortFile(inputPath, outputPath, tmpFolderPath);
	}
	
	//constructor for sorting files
	public FileSorter9(String inputPath, String outputPath, String tmpFolderPath, int[] columnsToSortFor,int chunkSize, String delimiter,boolean numericalSort) {
	...
	public FileSorter9(File folderToMerge, int[] columnsToSortFor,int chunkSize, String delimiter,boolean numericalSort) {
		this.columnsOfInterest = new HashSet<Integer>();
		for(int i = 0; i < columnsToSortFor.length; i++ ) {
			this.columnsOfInterest.add(columnsToSortFor[i]);
		}
		...
		try {
			...
			while((currentLine = br.getNextLine()) != null) {
				...
				if(++processedLines == this.linesPerChunkFile) {
					...
					currentStart = currentSparseLines.get(0).getFilePointer();
					currentLength = currentSparseLines.get(0).getLineLength();
					for(int i = 1; i < currentSparseLines.size(); i++) {
						
						if(currentSparseLines.get(i).getFilePointer() == (currentStart + currentLength)) {
							currentLength += currentSparseLines.get(i).getLineLength();
							
						}
						else {
							brChannel.transferTo(currentStart,currentLength, writeChannel);
							currentStart = currentSparseLines.get(i).getFilePointer();
							currentLength = currentSparseLines.get(i).getLineLength();
						}
